...
export function BenchmarkScene(): JSX.Element {
  return (
    <Canvas camera={{ position: [8, 6, 8], fov: 60 }} shadows data-testid="benchmark-canvas">
...
function BenchmarkSceneContent(): JSX.Element {
  ...
  return (
    <>
      ...
      {/* Test cube */}
      <mesh position={[0, 0.5, 0]} castShadow receiveShadow data-testid="benchmark-cube">
        <boxGeometry args={[1, 1, 1]} />
        <meshStandardMaterial color="orange" metalness={0.1} roughness={0.4} />
      </mesh>

      {/* Test sphere */}
      <mesh position={[2, 0.7, -2]} castShadow receiveShadow data-testid="benchmark-sphere">
      ...
      <group data-testid="benchmark-stairs">
        {[0, 1, 2, 3].map((step) => (
      ...
      <mesh position={[4, -2, 2]} data-testid="benchmark-shaft" receiveShadow>
