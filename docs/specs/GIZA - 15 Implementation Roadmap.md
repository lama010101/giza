...
| R16 | GitHub `quality` CI check fails because of account billing | High | High | **RESOLVED** on 2026-08-10: billing/spending limit restored; PR #30 CI (`quality`, `docs`, `label`) now passes, Vercel preview deployed, and `npm audit` remains at 0 vulnerabilities. The same pipeline passes locally and in clean `node:20` Docker clones. |
...
| R18 | Monument GPS positions use equirectangular approximation | Medium | Medium | ADR-0008 anchors the world origin at the Great Pyramid and derives other monument origins from WGS-84 using an equirectangular projection. This is adequate for visualization scale but may deviate from survey-grade UTM or a local Giza grid. Mitigation: replace `latLonToPlateauMeters` with UTM zone 36N or a published local grid if sub-meter accuracy becomes required for cross-monument measurements. |
...
| 2026-08-10 | 0.19 Draft | Closed M07-T10 Museum Mode: added `museumPaused` and `museumPresetName` to `useAppStore`, `src/scene/museumPresets.ts` with curated world-space camera presets for both monuments, `src/scene/MuseumLoop.tsx` driving the camera through presets with 2-second eased transitions and idle-attract pause/resume after 10 seconds of inactivity, `src/ui/MuseumOverlay.tsx` with large-type, touch-friendly pause/resume and exit controls, and CSS for full-screen kiosk presentation; wired `MuseumLoop` into `CameraRig.tsx` and `MuseumOverlay` into `Viewport.tsx`; `AppLayout.tsx` now hides header, side panels, and bottom toolbar in Museum mode. Tests added in `museumPresets.test.ts` and `MuseumOverlay.test.tsx`. All quality gates pass. |
