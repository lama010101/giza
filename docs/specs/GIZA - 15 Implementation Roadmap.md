...
| R17 | Residual `npm audit` vulnerabilities | Medium | High | 15 vulnerabilities remain after non-breaking `npm audit fix` (10 moderate, 4 high, 1 critical) in transitive dependencies of `@react-three/drei`, `@react-three/rapier`, `vitest`, `playwright`, and `lint-staged`. Full remediation needs `--force`/major-version updates and visual regression validation before release. |
